PROGRAM ThermWire
 ...
 ENERGY(2)=0.25D0-LAMBDA*LAMBDA/PHENERGY
  ...
  ! PLA RESULTS
  GAMMAL(1,1)=1.0*0.2D0 
  GAMMAL(1,2)=1.0*0.2D0*SQRT(b)
  GAMMAL(2,1)=1.0*0.2D0*SQRT(b)
  GAMMAL(C,C)=1.0*0.2D0*b

  GAMMAR(1,1)=1.0*0.2D0*b 
  GAMMAR(1,2)=1.0*0.2D0*SQRT(b)
  GAMMAR(2,1)=1.0*0.2D0*SQRT(b)
  GAMMAR(C,C)=1.0*0.2D0
...
!            END DO
           ...
           !Here we get the dressed greater and lesser green function GRGT,GRLT at w-R*w0
        ...
        DO R=-M,M
           ...
           !CALL THE LINEAR SYSTEM SOLVER TO GET DRESSED GA
           CALL SOLVER(A,TERM,X) 
        ...
        GAA=GRR+GLT-GGT 
!               write(8,*),W,Real(GRR(1,1)),Real((0,1)*GRR(1,1)),Real(GRR(2,2)),Real((0,1)*GRR(2,2))
!               write(9,*),W,Real(GAA(1,1)),Real((0,1)*GAA(1,1)),Real(GAA(2,2)),Real((0,1)*GAA(2,2))
!               write(81,*),W,Real(GRR(1,2)),Real((0,1)*GRR(1,2)),Real(GRR(2,1)),Real((0,1)*GRR(2,1))
!               write(91,*),W,Real(GAA(1,2)),Real((0,1)*GAA(1,2)),Real(GAA(2,1)),Real((0,1)*GAA(2,1))
!                   
!               write(82,*),W,Real(GLT(1,1)),Real((0,1)*GLT(1,1)),Real(GLT(2,2)),Real((0,1)*GLT(2,2))
!               write(92,*),W,Real(GGT(1,1)),Real((0,1)*GGT(1,1)),Real(GGT(2,2)),Real((0,1)*GGT(2,2))
!               write(83,*),W,Real(GLT(1,2)),Real((0,1)*GLT(1,2)),Real(GLT(2,1)),Real((0,1)*GLT(2,1))
!               write(93,*),W,Real(GGT(1,2)),Real((0,1)*GGT(1,2)),Real(GGT(2,1)),Real((0,1)*GGT(2,1))
                    
        SPECTRAL=(0,1)*(GGT-GLT)
        !SPECTRAL=(0,1)*(GRR-GAA)
        TRANSCOEFF=MATMUL(MATMUL(GAA,GAMMAR),MATMUL(GRR,GAMMAL))
        !TRANSCOEFF=MATMUL(GAMMAR,MATMUL(GRR,GAMMAL))
        !SPECTRAL=MATMUL(GGT,GAMMAL)
        !TRANSCOEFF=(0,1)*(GRR-GAA)
